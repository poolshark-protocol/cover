...
.setAction(async (args, hre) => {
  ...
  tokenAContract.mint(args.address, ethers.utils.parseUnits(args.amount, await tokenAContract.decimals()), {nonce: nonce});

  nonce += 1;

  tokenBContract.mint(args.address, ethers.utils.parseUnits(args.amount, await tokenBContract.decimals()), {nonce: nonce});
