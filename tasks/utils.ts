...
export async function getWalletAddress(
  ...
  return new hre.ethers.Wallet(privateKey !== undefined ? privateKey : "")
    .address;
}
...
export async function fundUser(hre: HardhatRuntimeEnvironment, token, userToFund, amount, needsEth = false) {
  let whaleAddress = token.whale;

  await hre.ethers.provider.send("hardhat_impersonateAccount", [
    whaleAddress,
  ]);
  const impersonatedAccount = await hre.ethers.provider.getSigner(
    whaleAddress
  );
  ...
  if (needsEth) {
    ...
    const ETHimpersonatedAccount = await hre.ethers.provider.getSigner(
      whaleAddress
    );
