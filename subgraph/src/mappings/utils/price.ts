...
import { BasePrice, CoverPool, Token } from '../../../generated/schema'
...
export function getEthPriceInUSD(): BigDecimal {
  ...
  let stablePool = CoverPool.load(STABLE_POOL_ADDRESS.toLowerCase()) // stable is token0
  if (stablePool !== null) {
...
export function findEthPerToken(token: Token, otherToken: Token): BigDecimal {
  ...
  if (STABLE_COINS.includes(token.id)) {
    ...
    for (let i = 0; i < whiteList.length; ++i) {
      ...
      let pool = CoverPool.load(poolAddress)
