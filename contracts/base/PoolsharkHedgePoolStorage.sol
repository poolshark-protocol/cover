...
abstract contract PoolsharkHedgePoolStorage is IPoolsharkHedgePoolStructs, PoolsharkHedgePoolErrors {
    ...
    uint128 public liquidity0;
    uint128 public liquidity1;
    ...
    uint256 public feeGrowthGlobal0Last;
    uint256 public feeGrowthGlobal1Last;

    uint128 internal token0ProtocolFee;
    uint128 internal token1ProtocolFee;

    uint160 internal sqrtPrice0; /// @dev Sqrt of price aka. √(y/x), multiplied by 2^96.
    uint160 internal sqrtPrice1; /// @dev Sqrt of price aka. √(y/x), multiplied by 2^96.
    
    int24 internal nearestTick0; /// @dev Tick that is just below the current price.
    int24 internal nearestTick1; /// @dev Tick that is just below the current price.

    mapping(int24 => Tick) public ticks;
    mapping(address => mapping(int24 => mapping(int24 => mapping(bool => Position)))) public positions;
